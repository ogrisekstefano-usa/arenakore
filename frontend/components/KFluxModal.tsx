/**
 * K-FLUX DETAIL MODAL — Build 31
 * ═══════════════════════════════
 * Blur-backed modal showing K-FLUX tier details:
 * - Vital K-Flux (Ciano) — guadagnato con auto-check-in giornaliero
 * - Perform K-Flux (Oro) — guadagnato con prestazioni e sfide
 * - Team K-Flux (Viola) — guadagnato con attività di squadra
 */
...
const TIERS: KFluxTier[] = [
  {
    key: 'vital', name: 'VITAL K-FLUX', icon: 'heart', color: CYAN, value: 0, goal: 1000,
    perks: [
      'Sblocca AI Matchmaker (500 K-Flux)',
      'Accesso Template Coach Certificati',
      'Partecipazione Sfide Agonistiche',
      'Custom Avatar Frame',
    ],
  },
  {
    key: 'perform', name: 'PERFORM K-FLUX', icon: 'trophy', color: GOLD, value: 0, goal: 100,
    perks: [
  ...
  {
    key: 'team', name: 'TEAM K-FLUX', icon: 'people', color: PURPLE, value: 0, goal: 50,
    perks: [
...
interface KFluxModalProps {
  visible: boolean;
  onClose: () => void;
  vitalFlux: number;
  performFlux: number;
  teamFlux: number;
  initialTier?: number; // 0 = vital, 1 = perform, 2 = team
}

export function KFluxModal({ visible, onClose, vitalFlux, performFlux, teamFlux, initialTier = 0 }: KFluxModalProps) {
  const [activeTier, setActiveTier] = React.useState(initialTier);

  React.useEffect(() => { setActiveTier(initialTier); }, [initialTier]);

  const values = [vitalFlux, performFlux, teamFlux];
