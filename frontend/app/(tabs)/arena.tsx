...
/**
 * ARENA TAB — Build 25 · FULL VISUAL RESTORATION
 * ═══════════════════════════════════════════════════
 * Hero Banner + KORE of the Day + Live Battles + Matchmaking AI + Elite Activity Feed
 * IRONCLAD network layer · Skeleton Screens · Retry on Error · In-Memory Cache
 */
import React, { useEffect, useState, useCallback } from 'react';
import {
  View, Text, StyleSheet, ScrollView, StatusBar,
  TouchableOpacity, Dimensions, ImageBackground, Alert, RefreshControl
} from 'react-native';
...
// ═══ IRONCLAD safe fetch ═══
async function safeFetch(fn: () => Promise<any>, cacheKey?: string): Promise<{ data: any; error: string | null }> {
  ...
  try {
    ...
    if (result && typeof result === 'object' && !result.detail && !result.error) {
      if (cacheKey) setCache(cacheKey, result);
      return { data: result, error: null };
    }
    return { data: null, error: result?.detail || result?.error || 'Dati non validi' };
  } catch (e: any) {
...
// ═══ RETRY BUTTON ═══
function RetryBlock({ error, onRetry }: { error: string; onRetry: () => void }) {
  return (
    <View style={retry$.wrap}>
      <Ionicons name="cloud-offline-outline" size={20} color="rgba(255,69,58,0.4)" />
      <Text style={retry$.text}>{error}</Text>
      <TouchableOpacity style={retry$.btn} onPress={onRetry} activeOpacity={0.8}>
        <Ionicons name="refresh" size={14} color="#000" />
        <Text style={retry$.btnText}>RIPROVA</Text>
      </TouchableOpacity>
    </View>
  );
}
const retry$ = StyleSheet.create({
  wrap: { marginHorizontal: 24, alignItems: 'center', gap: 10, paddingVertical: 24, backgroundColor: 'rgba(255,69,58,0.04)', borderRadius: 14, borderWidth: 1, borderColor: 'rgba(255,69,58,0.1)' },
  text: { color: 'rgba(255,69,58,0.5)', fontSize: 12, fontWeight: '600', textAlign: 'center', paddingHorizontal: 16 },
  btn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#FF453A', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8 },
  btnText: { color: '#000', fontSize: 12, fontWeight: '900', letterSpacing: 1.5 },
});
        ...
        <View style={hero$.statsRow}>
          {[
            ...
            <View key={i} style={hero$.statItem}>
              ...
              <Text style={hero$.statVal}>{st.val}</Text>
              <Text style={hero$.statLabel}>{st.label}</Text>
            </View>
          ))}
        </View>
      </LinearGradient>
...
const hero$ = StyleSheet.create({
  ...
  grad: { borderRadius: 20, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)' },
  ...
  statsRow: { flexDirection: 'row', borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.05)' },
  statItem: { flex: 1, alignItems: 'center', gap: 3, paddingVertical: 12, borderRightWidth: 1, borderRightColor: 'rgba(255,255,255,0.04)' },
  ...
  return (
    <Animated.View entering={FadeInDown.delay(100)} style={kotd$.container}>
      <ImageBackground source={{ uri: KORE_HERO }} style={kotd$.card} imageStyle={kotd$.cardImage}>
        <LinearGradient colors={['rgba(0,0,0,0)', 'rgba(5,5,5,0.6)', 'rgba(5,5,5,0.97)']} locations={[0, 0.45, 0.85]} style={kotd$.grad}>
          ...
          <View style={kotd$.bottom}>
            ...
            <View style={kotd$.detailsRow}>
              <View style={kotd$.avatar}><Text style={kotd$.avatarLetter}>T</Text></View>
              <View style={kotd$.info}><Text style={kotd$.sport}>{KORE_OF_DAY.sport} · RANK {KORE_OF_DAY.rank}</Text></View>
      ...
      {ELITE_FEED.map((item, idx) => {
        ...
        return (
          <Animated.View key={item.id} entering={FadeInDown.delay(idx * 90).duration(350)}>
            <ImageBackground source={{ uri: FEED_IMAGES[idx % FEED_IMAGES.length] }} style={ef$.card} imageStyle={ef$.cardImage}>
              <LinearGradient colors={['rgba(0,0,0,0)', 'rgba(5,5,5,0.55)', 'rgba(5,5,5,0.96)']} locations={[0, 0.3, 0.75]} style={ef$.cardGrad}>
                ...
                <View style={ef$.cardBottom}>
                  <View style={ef$.avatarWrap}><Text style={ef$.avatarLetter}>{item.athlete[0]}</Text></View>
...
function useCountdown(endsAt: string | null) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setMinsLeft(h * 60 + m);
      setRemaining(h > 0 ? `${h}h ${m}m` : m > 0 ? `${m}m ${sec}s` : `${sec}s`);
    };
    update();
...
function LiveBattleCard({ battle, onOpenDashboard }: { battle: any; onOpenDashboard: (id: string) => void }) {
  ...
  return (
    <Animated.View entering={FadeInDown.duration(300)} style={[wrc$.card, borderStyle]}>
      ...
      {isLastPush ? (
        <TouchableOpacity style={wrc$.lastPush} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {}); router.push('/(tabs)/nexus-trigger'); }} activeOpacity={0.85}>
          <Text style={wrc$.lastPushText}>LAST PUSH: SECURE THE VICTORY</Text>
        </TouchableOpacity>
      ) : battle.user_in_battle ? (
        <TouchableOpacity style={[wrc$.cta, isLosing && wrc$.ctaLosing]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium).catch(() => {}); router.push('/(tabs)/nexus-trigger'); }} activeOpacity={0.85}>
          <Ionicons name="flash-sharp" size={13} color={isLosing ? '#FFFFFF' : '#000000'} />
          <Text style={[wrc$.ctaText, isLosing && { color: '#FFFFFF' }]}>{isLosing ? 'RECUPERA — FALLO ORA' : 'CONTRIBUISCI ORA'}</Text>
        </TouchableOpacity>
      ) : null}
      {battle.user_in_battle && (
        <TouchableOpacity style={wrc$.dashBtn} onPress={() => onOpenDashboard(battle.id)} activeOpacity={0.75}>
...
function LiveBattleDashboard() {
  ...
  const load = useCallback(async () => {
    if (!token) return;
    setError(null);
    const { data, error: err } = await safeFetch(() => api.getLiveCrewBattles(token), 'live_battles');
    if (err) { setError(err); }
    else { setBattles(Array.isArray(data) ? data : []); }
    setLoading(false);
  }, [token]);
  ...
  return (
    <View style={lbd$.section}>
      ...
      <CrewBattleDashboard visible={!!dashboardBattleId} battleId={dashboardBattleId} onClose={() => setDashboardBattleId(null)} />
    </View>
  );
}
      ...
      {!isCertified && (
        <Animated.View entering={FadeInDown.duration(300)} style={s.urgencyBanner}>
          <View style={s.urgencyLeft}>
            ...
            <Text style={s.urgencyText}>Il tuo talento non è certificato. Completa il NÈXUS Scan per entrare nel Radar degli Scout.</Text>
          </View>
          <TouchableOpacity style={s.urgencyCta} onPress={() => router.push('/onboarding/choice')} activeOpacity={0.85}>
