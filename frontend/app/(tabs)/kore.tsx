/**
 * KORE TAB — Build 21 · STEP-BY-STEP · Hello World
    ...
    fetch(`${API}/users/me`, {
      ...
      .then(d => {
        const summary = {
          username: d?.username,
          role: d?.role,
          ak_credits: d?.ak_credits,
          sport: d?.sport,
          kore_id: d?.kore_id,
        };
        setApiResult(JSON.stringify(summary, null, 2));
      })
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.content}>
        ...
        <View style={s.divider} />
        <Text style={s.label}>API /users/me (JSON GREZZO)</Text>
        {loading ? (
        ...
        <View style={s.divider} />
        <Text style={s.footer}>KORE Tab operativa · Nessun crash atteso</Text>
      </ScrollView>
    </SafeAreaView>
  );
}
...
const s = StyleSheet.create({
  ...
  json: { color: '#FFD700', fontSize: 11, fontWeight: '500', fontFamily: 'monospace', lineHeight: 16 },
