/**
 * NEXUS TRIGGER — Build 21 · STEP-BY-STEP · Hello World
...
export default function NexusTrigger() {
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.content}>
        <Text style={s.title}>NEXUS COMMAND</Text>
        ...
        <View style={s.divider} />
        <Text style={s.label}>API /users/me</Text>
        {loading ? (
          <ActivityIndicator color="#00E5FF" style={{ marginTop: 12 }} />
        ) : (
          <Text style={s.json}>{apiResult}</Text>
        )}
        <View style={s.divider} />
        <Text style={s.footer}>Tab operativa · Nessun crash atteso</Text>
      </ScrollView>
    </SafeAreaView>
  );
}
...
const s = StyleSheet.create({
  ...
  json: { color: '#00E5FF', fontSize: 11, fontWeight: '500', fontFamily: 'monospace', lineHeight: 16 },
