/**
 * ISOLATION TEST — NEXUS TAB (EMPTY)
 * All heavy components disabled for crash isolation.
 */
import React from 'react';
import { View, Text, StyleSheet } from 'react-native';

export default function NexusTriggerScreen() {
  return (
    <View style={s.root}>
      <Text style={s.title}>NÈXUS</Text>
      <Text style={s.ok}>BOOT OK — TEST MODE</Text>
      <Text style={s.info}>Moduli pesanti disattivati</Text>
    </View>
  );
}

const s = StyleSheet.create({
  root: { flex: 1, backgroundColor: '#000', alignItems: 'center', justifyContent: 'center' },
  title: { color: '#00E5FF', fontSize: 28, fontWeight: '900', letterSpacing: 4 },
  ok: { color: '#00FF87', fontSize: 18, fontWeight: '700', marginTop: 16 },
  info: { color: '#666', fontSize: 13, marginTop: 8 },
});
