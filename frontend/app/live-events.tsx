...
export default function LiveEvents() {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${process.env.EXPO_PUBLIC_BACKEND_URL}/api/live-events`, {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      const res = await fetch(`${process.env.EXPO_PUBLIC_BACKEND_URL}/api/live-events/create`, {
        method: 'POST',
  ...
  const handleJoinEvent = async (eventId: string) => {
    try {
      const res = await fetch(`${process.env.EXPO_PUBLIC_BACKEND_URL}/api/live-events/${eventId}/join`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      if (res.ok) {
