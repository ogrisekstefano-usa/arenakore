...
export default function DuelSearch() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.EXPO_PUBLIC_BACKEND_URL}/api/duel/search?${params}`, {
  ...
  const handleChallenge = async (opponentId: string, opponentName: string) => {
    ...
    try {
      const res = await fetch(`${process.env.EXPO_PUBLIC_BACKEND_URL}/api/pvp/challenge`, {
        method: 'POST',
