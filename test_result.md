...
test_plan: "PRE-FLIGHT BUILD 36 — K-Flux Tiered Separation Test. (1) Login: POST /api/auth/login {email: ogrisek.stefano@gmail.com, password: Founder@KORE2026!}. (2) GET /api/flux/balance — verify vital, perform, team fields. (3) GET /api/flux/wallet — verify balance.green, balance.cyan, balance.amber, spendable.burn_priority=['green','cyan','amber']. (4) GET /api/me — verify vital_flux, master_flux, diamond_flux fields are present. (5) POST /api/checkin/hub/{hub_id}/generate-qr — get a hub QR (use any valid hub). (6) Note down flux balance, then scan QR: POST /api/checkin/scan {qr_payload from step 5} — verify vital_flux was incremented (GREEN tier). Check flux/balance again to verify vital went up. (7) Quick Training: POST /api/training/quick {category: 'strength', duration_seconds: 600, rpe: 7, exercises: [{name: 'Push-ups', sets: 3, reps: 10}]} — verify vital_flux incremented. (8) Challenge complete simulation: POST /api/challenges/complete {performance_score: 85, duration_seconds: 45, reps_completed: 20, quality_score: 90, ai_feedback_score: 85} — verify flux was awarded to diamond_flux (amber tier since quality_score>0 = nexus bio). (9) GET /api/marketplace/offers — verify wallet reflects updated balances."

agent_communication:
    - agent: "main"
      message: "PRE-FLIGHT BUILD 36 FIX: Fixed K-Flux tiered separation. award_ak_credits() now accepts flux_tier param (auto/green/cyan/amber). Green→vital_flux (checkin, quick training), Cyan→master_flux (PvP, crew battles, standard challenges), Amber→diamond_flux (NEXUS bio-verified sessions). Fixed flux_vital→vital_flux field name in quick training. Added vital_flux to /api/me response. Flux balance endpoint correctly maps all 3 tiers."
